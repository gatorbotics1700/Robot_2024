...
public class ElevatorSubsystem {
    ...
    private void elevatorDeadband(double desiredTicks){
        if (Math.abs(desiredTicks - elevatorMotor.getRotorPosition().getValue()) > DEADBAND){
            elevatorMotor.set(desiredTicks); 
        } else {
            elevatorMotor.set(0); // deleted PercentOutput because of V6 change
        }
    }
    ...
    private void manualElevator(){
        if(elevatorMotor.getRotorPosition().getValue() <= MAX_HEIGHT_INCHES * ELEVATOR_TICKS_PER_INCH /*&&
    ...
    public boolean isAtMid(){
        return Math.abs(elevatorMotor.getRotorPosition().getValue()-MID_HEIGHT_INCHES*ELEVATOR_TICKS_PER_INCH)<DEADBAND;
    }

    public boolean isAtLow(){
        return Math.abs(elevatorMotor.getRotorPosition().getValue()-LOW_HEIGHT_INCHES*ELEVATOR_TICKS_PER_INCH)<DEADBAND;
    }

    public boolean isAtShelf(){
        return Math.abs(elevatorMotor.getRotorPosition().getValue()-SHELF_HEIGHT_INCHES*ELEVATOR_TICKS_PER_INCH) < DEADBAND;
    }

    public boolean isAtZero(){
        return elevatorMotor.getRotorPosition().getValue() < DEADBAND;
    }
